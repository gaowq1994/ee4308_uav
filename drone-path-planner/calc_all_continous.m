...
elseif(taskChoice == 2)
    fileID = fopen('coordinates_task1_03ms2.txt','r'); %changed from 03ms2 to 03ms
...
if (taskChoice == 1 || taskChoice == 3)
    ...
    %------------------ point B to C --------------------%
    mode = 1; % Decending in the last 4 waypoints
...
%------------------ Landing at point C --------------------%
mode = 2;
path_C_to_ground = [WayPoints(end,1),WayPoints(end,2), delta+0.5;
                    WayPoints(end,1),WayPoints(end,2), delta];
